#!/usr/bin/env python3
"""
Test tworzenia rezerwacji dla Ariny na 12:00 26.10.2025
"""
import os
import logging
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('arina_booking_test.log')
    ]
)
...
def test_arina_booking():
    """Test tworzenia rezerwacji dla Ariny"""
    log.info("🚀 STARTING ARINA BOOKING TEST")
    ...
    try:
        ...
        # Pobieranie masterów - znajdź Arinę
        log.info("👥 Getting masters...")
        masters = client.company_masters(company_id)
        arina = None
        for m in masters["data"]:
            if "арина" in m["name"].lower():
                arina = m
                break
        master = arina if arina else masters["data"][0]
        ...
        new_client = client.create_client(
            company_id,
            name="Test Client Arina",
            phone=phone,
            email="test@example.com",
            comment="Test client for Arina booking"
        )
        client_id = new_client["data"]["id"]
        log.info(f"✅ Created client ID: {client_id}")
        
        # Tworzenie rezerwacji dla Ariny na 12:00 26.10.2025
        log.info("📝 Creating booking for Arina...")
        booking = client.create_record(
            ...
            client_id=client_id,
            comment="Test booking for Arina - 26.10.2025 12:00",
...
if __name__ == "__main__":
    success = test_arina_booking()
    if success:
        print("✅ Arina booking test completed successfully!")
        print("📝 Check your YClients system for the new booking!")
    else:
        print("❌ Arina booking test failed!")
